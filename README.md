...
| `/wt-auto-review` | worktree | AI review for `/wt-split` child tasks: a fresh subagent judges the diff against the child issue's acceptance criteria; a PASS satisfies the `/wt-merge` approval gate and the merge proceeds unattended. Falls back to human review (`/wt-review`) after two failed rounds |
