...
ok22=1
for s in worktree-parallel wt wt-detail wt-split wt-review wt-auto-review wt-merge wt-clean wt-ask local-artifact; do
  [ -f "$H22/.claude/skills/$s/SKILL.md" ] || ok22=0
done
if [ "$ok22" -eq 1 ]; then
  pass "install: skills 10 個を ~/.claude/skills に配置する"
else
  fail "install: skills 10 個を ~/.claude/skills に配置する"
fi
